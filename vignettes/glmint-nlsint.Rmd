...
$$
  g[E(Y_i)] = \eta_i
$$
where $g$ is the link function and $\eta_i = \beta_0 + \beta_1 x_{i1} + \cdots + \beta_p x_{ip}$ is the linear predictor. The `predict` function computes $\hat\eta_i = \hat\beta_0 + \hat\beta_1 x_{i1} + \cdots + \hat\beta_p x_{ip}$ and the standard error of $\hat\eta_i$. The confidence interval for $g[E(Y_i)]$ is then 
$$
  \hat\eta_i \pm t\sqrt{\widehat{\text{Var}}(\hat\eta_i)},
$$
and the confidence interval for $E(Y_i)$ is then
$$
...
For an example consider a logistic regression model for the `rotifer` data, using quasilikelihood to account for the overdispersion. The model is estimated as
```{r}
