...
#' @param delta Divisor for the marginal effect (default is one).
...
margeff <- function(model, a, b, df, cnames, delta = 1, level = 0.95, fcov = vcov, ...) {
  ...
  pe <- (predict(model, as.data.frame(a), type = "response") - 
    predict(model, as.data.frame(b), type = "response"))/delta
  gr <- numDeriv::jacobian(f, coef(model), model = model, a = a, b = b, delta = delta)
